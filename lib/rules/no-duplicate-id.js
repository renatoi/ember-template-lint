...
module.exports = class NoDuplicateId extends Rule {
  ...
  visitor() {
    ...
    function handleCurlyNode(node, path) {
      let idValue = this.getHashArgIdValue(node, 'elementId');
      if (idValue && !hasControlFlowHelperParent(node, path)) {
    ...
    // Check to see if node is contained within a control flow helper BlockStatement
    function hasControlFlowHelperParent(node, path) {
      let parents = [...path.parents()];
      let result = parents.find(
        (parent) =>
          AstNodeInfo.isBlockStatement(parent.node) && AstNodeInfo.isControlFlowHelper(parent.node)
      );
      return result;
    }
    ...
    return {
      AttrNode(node, path) {
        ...
        if (hasControlFlowHelperParent(node, path) || !isValidIdAttrNode(node)) {
          return;
        }
